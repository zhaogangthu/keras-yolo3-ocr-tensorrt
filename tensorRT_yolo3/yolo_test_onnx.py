# -*- coding: utf-8 -*-
# @Time    : 2020/4/16 15:29
# @Author  : zhaogang

# -*- coding: utf-8 -*-
# @Time    : 2020/4/15 14:58
# @Author  : zhaogang
...
class YoloPredict(object):
    def __init__(self,path):
        ...
        with graph.as_default():
            ...
            self.out1 = Input(batch_shape=(1, 76, 76, 21))
            self.out2 = Input(batch_shape=(1, 38, 38, 21))
            self.out3 = Input(batch_shape=(1, 19, 19, 21))

            self.box_score = self.box_layer([self.out1,self.out2,self.out3, self.image_shape, self.input_shape], anchors, num_classes)

    def init_tensorrt_engine(self):
        self.TRT_LOGGER = trt.Logger(trt.Logger.WARNING)
        engine=self.get_engine("tensorRT_yolo3/model/weights-densent-05-loss_344.7720.h5.onnx","tensorRT_yolo3/model/engine.onnx")
    ...
    def get_engine(self,onnx_file_path, engine_file_path=""):
        ...
        def build_engine():
            ...
            with trt.Builder(self.TRT_LOGGER) as builder, builder.create_network(
                    common.EXPLICIT_BATCH) as network, trt.OnnxParser(network, self.TRT_LOGGER) as parser:
    ...
    def predict(self,img,prob = 0.05):
        ...
        output_shapes = [(1,76,76,21), (1,38,38,21), (1,19,19,21)]
        ...
        with graph.as_default():
            ...
            try:
                box, scores = sess.run(
                    [self.box_score],
                    feed_dict={
                        self.out1: trt_outputs[0],
                        self.out2: trt_outputs[1],
                        self.out3: trt_outputs[2],
